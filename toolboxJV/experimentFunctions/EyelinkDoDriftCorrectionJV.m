function success=EyelinkDoDriftCorrectionJV(el, x, y, draw, allowsetup)
...
%def is don't clear instead of clear!!!!!!!!!    JV adaption
...
while 1	
	if Eyelink('IsConnected')==el.notconnected   % Check link often so we don't lock up if tracker lost
		...
		success=0;
        disp('Gooit ie hem er hier uit?');
		return;
	end;
	% DRIFT CORRECTION */
	% 3rd argument would be 0 to NOT draw a target */
	% fprintf('drifcorr at % d %d\n', x, y );
	error = EyelinkDoDriftCorrectJV(el, x, y, draw, allowsetup);

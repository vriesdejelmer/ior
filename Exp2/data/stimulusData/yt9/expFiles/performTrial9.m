function [succes, trialInf] = performTrial8(w,el,expProps,trialInf,trial,condition, subCondition,doDriftCorrect,rotationPosition)                                         
    ...
    if condition == CONDITION_NO_IOR,
        [~, offsetFixation, onsetTarget, offsetDistractor] = showTargetDistractor(w, elementX(antiLocs(1)), elementY(antiLocs(1)), elementX(antiLocs(2)), elementY(antiLocs(2)),expProps,antiLocs,targetColor,distractorColor,subCondition,targetOnsetTime);
    else
        [~, offsetFixation, onsetTarget, offsetDistractor] = showTargetDistractor(w, elementX(cueLocs(1)), elementY(cueLocs(1)), elementX(cueLocs(2)), elementY(cueLocs(2)),expProps,cueLocs,targetColor,distractorColor,subCondition,targetOnsetTime);
    end
    ...
    trialInf.offsetDist     = offsetDistractor - trialInf.stimulusTime; % correction for start stamp    
...
function [vblTime, offsetFixation, onsetTargets, offsetDistractor] = showTargetDistractor(w, cueX1, cueY1, cueX2, cueY2,expProps,cueNum,targetColor,distractorColor,subCondition,cueOnsetTime)
    ...
    [vblTime, offsetDistractor] = Screen('Flip', w,cueOnsetTime+0.05,1);

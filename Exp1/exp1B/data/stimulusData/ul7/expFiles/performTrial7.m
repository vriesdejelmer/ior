function [succes, trialInf] = performTrial7(w,el,expProps,trialInf,trial,condition,subCondition,doDriftCorrect,rotationPosition)                                         
    ...
    [sysTimeCue, cueOnsetStamp]     = showDoubleCue(w, elementX(cueLocs(1)), elementY(cueLocs(1)), elementX(cueLocs(2)), elementY(cueLocs(2)),expProps,cueLocs,expProps.cueColor,cueOnsetTime);
    Eyelink('message', 'SYNCTIME');	 	 % zero-plot time for EDFVIEW
    trialInf.cueTime                = cueOnsetStamp - trialInf.stimulusTime; % correction for start stamp
    
        %stap 3 verwijder cues en blob fixation
    cueOffsetTime                   = sysTimeCue + expProps.cueOnTime;
    [sysTime, cueOffsetStamp, fixationIncrease, fixationDecrease]        = clearDoubleCue(w, elementX(cueLocs(1)), elementY(cueLocs(1)), elementX(cueLocs(2)), elementY(cueLocs(2)),expProps,cueOffsetTime,trialInf);
    ...
    trialInf.fixationDecrease       = fixationDecrease - trialInf.stimulusTime;
        
        %stap 4 voeg target en distractor ring toe
    targetOnsetTime    = sysTimeCue +  subCondition;
    if condition == CONDITION_IOR,
        [sysTime, targetTimeStamp] = showTargetDistractor(w, elementX(cueLocs(1)), elementY(cueLocs(1)), elementX(cueLocs(2)), elementY(cueLocs(2)),expProps.targetContrast,expProps.distractorContrast,expProps,cueLocs,targetOnsetTime);
    else
        [sysTime, targetTimeStamp] = showTargetDistractor(w, elementX(antiLocs(1)), elementY(antiLocs(1)), elementX(antiLocs(2)), elementY(antiLocs(2)),expProps.targetContrast,expProps.distractorContrast,expProps,antiLocs,targetOnsetTime);
    end
...
function [vblTime onsetScreenFlip] = showTargetDistractor(w, cueX1, cueY1, cueX2, cueY2,stim1Color,stim2Color,expProps,cueNum,cueOnsetTime)
    ...
    Screen('FillOval', w, expProps.backgroundColor,[expProps.startX-expProps.cueSize expProps.startY-expProps.cueSize expProps.startX+expProps.cueSize expProps.startY+expProps.cueSize], expProps.cueThickness);
        ...
        %draw target
    Screen('FrameOval', w, stim1Color*255,[cueX1-expProps.elementRadius cueY1-expProps.elementRadius cueX1+expProps.elementRadius cueY1+expProps.elementRadius],expProps.elementThickness);
        
        %draw distractor
    Screen('FrameOval', w, stim2Color*255,[cueX2-expProps.elementRadius cueY2-expProps.elementRadius cueX2+expProps.elementRadius cueY2+expProps.elementRadius],expProps.elementThickness);
...
function [vblTime, onsetScreenFlip, fixationIncrease, fixationDecrease] = clearDoubleCue(w, cueX1, cueY1, cueX2, cueY2,expProps,cueOffsetTime,trialInf)
